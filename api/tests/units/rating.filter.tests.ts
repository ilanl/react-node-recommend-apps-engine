...
import { ratingFilter } from '../../src/recommendations/filters/rating';

describe("Rating Filter", () => {  
  
  it("should return apps above min rating", async (done) => {
    ...
    const RATING = 4;
    let target = ratingFilter;
    let results = apps.filter((i: IAppInfo) => target(<IQueryFilter>{ rating: RATING }, i));
    expect(results.length).toEqual(1);
    expect(results[0].rating).toEqual(4.49);
    done();
  });
});